...
def format_risk_table(metrics: Dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("Annualised Return",      f"{t.get('ann_return', 0)*100:+.2f} %"),
        ("Annualised Volatility",  f"{t.get('ann_volatility', 0)*100:.2f} %"),
        ("Sharpe Ratio",           f"{t.get('sharpe', 0):.3f}  {sharpe_badge(t.get('sharpe', 0))}"),
        ("Sortino Ratio",          f"{t.get('sortino', 0):.3f}"),
        ("Max Drawdown",           f"{t.get('max_drawdown', 0)*100:.2f} %  {mdd_badge(t.get('max_drawdown', 0))}"),
        ("VaR 95 % (1-day)",       f"{t.get('var_95', 0)*100:.2f} %  {var_badge(t.get('var_95', 0))}"),
        ("Calmar Ratio",           f"{t.get('calmar', 0):.3f}"),
        ("Risk Level",             risk_label_emoji(t.get("risk_label", "—"))),
        ("Risk-Free Rate",         f"{t.get('risk_free_rate', 0.05)*100:.1f} %"),
    ]
    header = (
...
def risk_metrics_to_dict(metrics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "Ann Return %":  round(metrics.get("ann_return", 0) * 100, 2),
        "Volatility %":  round(metrics.get("ann_volatility", 0) * 100, 2),
        "Sharpe":        metrics.get("sharpe", 0),
        "Sortino":       metrics.get("sortino", 0),
        "Max DD %":      round(metrics.get("max_drawdown", 0) * 100, 2),
        "VaR 95% %":     round(metrics.get("var_95", 0) * 100, 2),
