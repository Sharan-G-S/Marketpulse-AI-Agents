...
def calculate_fibonacci_levels(price_history: List[Dict]) -> Dict[str, float]:
    ...
    if not price_history:
        return {}
    highs = [
        float(r.get("high")) if r.get("high") is not None else float(r.get("close", 0) or 0)
        for r in price_history
    ]
    lows = [
        float(r.get("low")) if r.get("low") is not None else float(r.get("close", 0) or 0)
        for r in price_history
    ]
    max_high = max(highs)
    min_low = min(lows)
    ...
    levels["0.618"] = round(max_high - 0.618 * diff, 2)
    levels["0.786"] = round(max_high - 0.786 * diff, 2)
    return levels
